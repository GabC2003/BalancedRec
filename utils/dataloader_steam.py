...
class Dataloader_steam_filtered(DGLDataset):
    ...
    def build_dataloader(self, args, graph):
        ...
        # 创建一个简单的 Dataset 类
        class SimpleDataset:
            def __init__(self, graph, train_id):
                self.graph = graph
                self.train_id = train_id
            def __len__(self):
                return len(self.train_id)
            def __getitem__(self, idx):
                return self.graph, self.train_id[idx]

        dataset = SimpleDataset(graph, train_id)
    
        # 使用 DataLoader 替代 GraphDataLoader
        dataloader = torch.utils.data.DataLoader(
            dataset,
            batch_size=args.batch_size,
            shuffle=True,
            num_workers=2,
            collate_fn=lambda batch: dgl.batch([b[0] for b in batch])
        )
        return dataloader
